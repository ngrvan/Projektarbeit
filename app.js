...
const start = async () => {
  try {
    await connectDB();
    app.listen(port, () => {
      console.log(`Server läuft auf ${api}`);
    });
  } catch (error) {
    console.log(error);
  }
...
//REST API Funkion
app.get("/", (req, res) => {
  res.redirect("/alle_auftraege");
});

app.get("/alle_auftraege", (req, res) => {
  res.render("index", { mytite: "Home" });
});

app.get("/add-neue-auftrag", (req, res) => {
  res.render("add-neue-auftrag", { mytite: "Neuer-Auftrag" });
});
